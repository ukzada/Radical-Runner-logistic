/**
 * Optional database seed script — Radical Runner Logistics demo dataset.
 *
 * The application itself does NOT use or require seed data: a fresh
 * deployment starts with an empty database. Run this script manually
 * only if you want demo accounts, companies, drivers and loads:
 *
 *   npm run db:seed        # runs: npx tsx prisma/seed.ts
 *
 * Demo credentials created by this script:
 *   Admin        admin@rrl.com        / admin123
 *   Dispatcher   dispatcher1@rrl.com  / dispatch123  (10% fee)
 *   Dispatcher   dispatcher2@rrl.com  / dispatch123  (8% fee)
 *   Dispatcher   dispatcher3@rrl.com  / dispatch123  (12% fee)
 *   Owner        owner@rrl.com        / owner123     (ABC Logistics)
 */
...
async function main() {
  ...
  if (existingAdmin) {
    console.log('Database already contains seed data (admin@rrl.com exists) — skipping.');
    return;
  }
  ...
  // 2. Dispatchers (with commission percentages)
  const dispatchers = await Promise.all([
    db.user.create({
      data: { email: 'dispatcher1@rrl.com', passwordHash: dispatcherPasswordHash, name: 'Sarah Johnson', phone: '(555) 201-1001', role: 'DISPATCHER', isActive: true, feePercentage: 10 },
    }),
    db.user.create({
      data: { email: 'dispatcher2@rrl.com', passwordHash: dispatcherPasswordHash, name: 'Mike Chen', phone: '(555) 201-1002', role: 'DISPATCHER', isActive: true, feePercentage: 8 },
    }),
    db.user.create({
      data: { email: 'dispatcher3@rrl.com', passwordHash: dispatcherPasswordHash, name: 'Lisa Martinez', phone: '(555) 201-1003', role: 'DISPATCHER', isActive: true, feePercentage: 12 },
    }),
  ]);
  ...
  // 3b. Company owner for ABC Logistics (COMPANY_OWNER portal demo account)
  await db.user.create({
    data: {
      email: 'owner@rrl.com',
  ...
  await Promise.all([
    ...
    // Tyler Jackson (drivers[7]) is intentionally unassigned
  ]);

  // 7. Loads
  // IN_TRANSIT loads get a deliveryDate ~20-24h out so the delivery-date
  // alarm system (scans the next 26h) has demo data immediately.
  ...
  const loadsData = [
    ...
    { driverId: drivers[1].id, createdBy: dispatchers[0].id, status: 'IN_TRANSIT', pickupCity: 'Jacksonville', pickupState: 'FL', deliveryCity: 'Charlotte', deliveryState: 'NC', loadPrice: 2800, commodity: 'Produce', loadDate: new Date('2025-06-10'), pickupDate: new Date('2025-06-10'), deliveryDate: hoursFromNow(20) },
    ...
    { driverId: drivers[2].id, createdBy: dispatchers[1].id, status: 'IN_TRANSIT', pickupCity: 'Houston', pickupState: 'TX', deliveryCity: 'Phoenix', deliveryState: 'AZ', loadPrice: 7200, commodity: 'Machinery', loadDate: new Date('2025-06-11'), pickupDate: new Date('2025-06-11'), deliveryDate: hoursFromNow(22) },
    ...
    { driverId: drivers[4].id, createdBy: dispatchers[2].id, status: 'IN_TRANSIT', pickupCity: 'Cincinnati', pickupState: 'OH', deliveryCity: 'Nashville', deliveryState: 'TN', loadPrice: 2100, commodity: 'Chemicals', loadDate: new Date('2025-06-15'), pickupDate: new Date('2025-06-15'), deliveryDate: hoursFromNow(24) },
  ];
  ...
  console.log(`Seed complete: 5 users, 3 companies, 7 MCs, 8 drivers, ${loads.length} loads.`);
  console.log('Demo logins: admin@rrl.com/admin123 · dispatcher1@rrl.com/dispatch123 · owner@rrl.com/owner123');
}
