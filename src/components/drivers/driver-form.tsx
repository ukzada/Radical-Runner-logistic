...
export function DriverForm({ open, onClose, editId, onSuccess, defaultCompanyId }: DriverFormProps) {
  const queryClient = useQueryClient();
  const { register, handleSubmit, reset, setValue, watch, formState: { errors } } = useForm<DriverFormValues>({
    resolver: zodResolver(driverSchema),
    defaultValues: { firstName: '', lastName: '', email: '', phone: '', cdlNumber: '', cdlState: '', cdlExpiration: '', status: 'ACTIVE', notes: '', mcId: '', companyId: defaultCompanyId || '', truckType: '' },
  });
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className='max-w-lg max-h-[90vh] overflow-y-auto'>
        ...
        <form onSubmit={handleSubmit((v) => mutation.mutate(v))} className='space-y-4'>
          ...
          <div className='space-y-1'>
            <Label>Company</Label>
            <Select value={watch('companyId') || '_none'} onValueChange={(v) => setValue('companyId', v === '_none' ? '' : v)}>
          ...
          <div className='space-y-1'>
            ...
            <Input {...register('truckType')} placeholder='e.g. Dry Van, Reefer, Flatbed' />
          </div>
